...
The `ControlCore` class scans the plugin package using `ModComponentScanner` and registers any concrete classes that implement `IModCommand` or `IModEvent`.
...
1. `ControlCore.setup()` calls `registerDynamicComponents()`.
